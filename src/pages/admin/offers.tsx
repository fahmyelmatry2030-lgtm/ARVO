            ...
            <div className="space-y-12">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8 border-b border-white/5 pb-12">
                    <div>
                        ...
                        <h1 className="text-4xl md:text-6xl font-black text-white leading-none tracking-tighter uppercase italic">إدارة العروض</h1>
                    </div>
                    <button
                    ...
                    <AnimatePresence>
                        {offers.length === 0 ? (
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="text-center py-40 bg-white/[0.02] border border-white/5 rounded-[3rem]"
                            >
                                {React.createElement(FiTag as any, { className: "w-16 h-16 text-white/10 mx-auto mb-6" })}
                                <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.8em]">لا توجد عروض حالياً</span>
                            </motion.div>
                        ) : (
                            offers.map((offer: any, index: number) => (
                                <motion.div
                                    ...
                                    className="group bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 hover:border-amber-500/30 transition-all duration-500"
                                >
                                    <div className="flex items-center gap-8 w-full md:w-auto">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-4 mb-2">
                                                <h3 className="text-2xl font-black text-white italic tracking-tighter">{offer.title}</h3>
                                            ...
                                            <p className="text-white/40 font-bold text-sm mb-4 leading-relaxed max-w-md">{offer.description}</p>
                                    ...
                                    <div className="flex items-center gap-4 w-full md:w-auto justify-end">
                                        <button className="flex-1 md:flex-none h-14 px-8 bg-white/[0.05] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white hover:text-black transition-all">
                                            تعديل
                                        </button>
                                        <button
                ...
                {/* Info Card */}
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-[3rem] p-10 flex flex-col md:flex-row items-center gap-10">
                    <div className="text-6xl">💡</div>
                    <div>
                        <h4 className="text-xl font-black text-white mb-2 italic">نصيحة للإدارة</h4>
                        <p className="text-white/60 font-bold leading-relaxed">العروض الموسمية تزيد من نسبة المبيعات بمعدل 40%. تأكد من جدولة عروضك القادمة قبل فترة كافية لجذب انتباه العملاء.</p>
                    </div>
                </div>
            </div>
        </AdminLayout>
